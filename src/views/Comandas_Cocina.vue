<template>
  <div id="app">
    ...
    <h2 v-if="comandas.length==0">No hay comandas para la cocina....</h2>

    <div v-for="comanda in comandas" v-bind:key="comanda.id">
        
        <div v-if="comanda.Completado==false" class="com">
          <hr>
          <h3>{{comanda.Mesa}}</h3>
  ...
  methods: {
    completar: function(producto){
      if(producto.Completado==false){
          db.collection("Cocina").doc(producto.id).set({
            Completado : true,
            Hora : producto.Hora,
            Mesa : producto.Mesa,
            Zona: producto.Zona,
  ...
  created:function(){
    ...
    mounted(){
      if(sessionStorage.camarero=="Admin"){
         ...
         this.$bind("comandas",db.collection("Cocina"));
        },
  computed: {
